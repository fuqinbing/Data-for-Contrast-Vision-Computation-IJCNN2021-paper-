...
namespace EMD
{
    ...
    internal class FlyMotionVision
    {
        ...
        /// <summary>
        /// Coefficient in contrast normalisation field
        /// </summary>
        ...
        /// <summary>
        /// Coefficient of parallel contrast pathway in summation
        /// </summary>
        ...
        /// <summary>
        /// Visual Projection Neurons - LPLC2
        /// </summary>
        ...
        protected double[,] Gaussian_Filter(double[,,] input, int gauss_width1, double[,] gauss_kernel1, int gauss_width2, double[,] gauss_kernel2, int t)
        {
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    // subtraction
                    result[y, x] = output1[y, x] - output2[y, x];
        ...
        public void FlyMotionVision_LPTC_And_Contrast(byte[,,] img1 /*first input image*/, byte[,,] img2 /*second input image*/, int t /*time*/)
        {
            ...
            if (blockContrast)
            {
                ...
                for (int y = sd; y < height - sd; y++)
                {
                    for (int x = sd; x < width - sd; x++)
                    {
                        ...
                        LPTCs[0] += (GELU_Fast(PolarityChannelSummation(ReLU(T4s[y, x, cur_t, 0]), ReLU(T5s[y, x, cur_t, 0]), exp_ON, exp_OFF)));
                        LPTCs[1] += (GELU_Fast(PolarityChannelSummation(ReLU(T4s[y, x, cur_t, 1]), ReLU(T5s[y, x, cur_t, 1]), exp_ON, exp_OFF)));
                        LPTCs[2] += (GELU_Fast(PolarityChannelSummation(ReLU(T4s[y, x, cur_t, 2]), ReLU(T5s[y, x, cur_t, 2]), exp_ON, exp_OFF)));
                        LPTCs[3] += (GELU_Fast(PolarityChannelSummation(ReLU(T4s[y, x, cur_t, 3]), ReLU(T5s[y, x, cur_t, 3]), exp_ON, exp_OFF)));
                    }
                }
            }
            else
            {
                ...
                for (int y = sd; y < height - sd; y++)
                {
                    for (int x = sd; x < width - sd; x++)
                    {
                        ...
                        /*
                        T4_mag[y, x] = Math.Sqrt(Math.Pow(T4s[y, x, cur_t, 0] - T4s[y, x, cur_t, 1], 2) + Math.Pow(T4s[y, x, cur_t, 2] - T4s[y, x, cur_t, 3], 2));
                        T5_mag[y, x] = Math.Sqrt(Math.Pow(T5s[y, x, cur_t, 0] - T5s[y, x, cur_t, 1], 2) + Math.Pow(T5s[y, x, cur_t, 2] - T5s[y, x, cur_t, 3], 2));
                        if (T4_maxMag < T4_mag[y, x])
                            T4_maxMag = T4_mag[y, x];
                        if (T5_maxMag < T5_mag[y, x])
                            T5_maxMag = T5_mag[y, x];
                        T4_dir[y, x] = Math.Atan2(T4s[y, x, cur_t, 2] - T4s[y, x, cur_t, 3], T4s[y, x, cur_t, 0] - T4s[y, x, cur_t, 1]) * 180 / Math.PI;
                        T5_dir[y, x] = Math.Atan2(T5s[y, x, cur_t, 2] - T5s[y, x, cur_t, 3], T5s[y, x, cur_t, 0] - T5s[y, x, cur_t, 1]) * 180 / Math.PI;
                        */
            ...
            //Motion signal maps
            //T4_msm = MotionSignalMap(T4_dir, T4_mag, T4_maxMag);
            //T5_msm = MotionSignalMap(T5_dir, T5_mag, T5_maxMag);
        ...
        /// <summary>
        /// Fly motion vision neural network model for LPLC2(ultra-selective looming) plus contrast vision
        /// </summary>
        /// <param name="img1">input grayscale image</param>
        /// <param name="img2">input grayscale image</param>
        /// <param name="t">time</param>
        public void FlyMotionVision_LPLC2_And_Contrast(byte[,,] img1 /*first input image*/, byte[,,] img2 /*second input image*/, int t /*time*/)
        {
            // Init and reset some states at every time step
            int cur_t = t % 2;
            int pre_t = (t - 1) % 2;
